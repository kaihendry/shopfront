...
#		prodprice=$(grep -m 1 -B1 "$sku" public/*/index.json | head -n 1 | cut -d'"' -f4)
#		curl https://api.stripe.com/v1/prices -u "${STRIPE_SECRET}:" -d product="$prodid" -d unit_amount="$prodprice" -d currency="${CURRENCY}"
#		echo -n "setting price for:"
#		echo "$sku"
#	done
fi
